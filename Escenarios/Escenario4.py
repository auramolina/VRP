...
locations["CI"]=E4.add_depot(
    x=float(CI["lon"]),
    y=float(CI["lat"]),
    name=CI["Nombre"]
)
locations["CD"]=E4.add_depot(
    x=float(CD["lon"]),
    y=float(CD["lat"]),
    name=CD["Nombre"]
)
...
for _, row in coords.iterrows():
    ...
    if planta in [loc.name for loc in E4.depots]:
        continue    
    ...
    if delivery > 0:
        ...
        for k, part in enumerate(parts, start=1):
            ...
            locations[name_d] = E4.add_client(
                x=float(row["lon"]),
                y=float(row["lat"]),
                delivery=[part],
                pickup=[],
    ...
    if pickup > 0:
        ...
        tw_e = 0
        tw_l = 1440 * SCALE
        locations[name_p] = E4.add_client(
            x=float(row["lon"]),
            y=float(row["lat"]),
            delivery=[],
            pickup=[pickup],
...
#--------------------------------------------  
# ====== Solución ======
res = E4.solve(MaxRuntime(300))
print(res.best)
solution = res.best
with open("Escenarios/Res4.pkl", "wb") as f:
    pickle.dump(res, f)
#--------------------------------------------  
# ====== Mapa Rutas ======
m = E4
