...
locations["CI"]=E3.add_depot(
    x=float(CI["lon"]),
    y=float(CI["lat"]),
    name=CI["Nombre"]
)
locations["CD"]=E3.add_depot(
    x=float(CD["lon"]),
    y=float(CD["lat"]),
    name=CD["Nombre"]
)
...
for _, row in coords.iterrows():
    ...
    if planta in ("A6", "42"): 
        ...
        tw_e = 0
        tw_l = 1440 * SCALE
    locations[planta] = E3.add_client(
        x=float(row["lon"]),
        y=float(row["lat"]),
...
#--------------------------------------------  
# Guardar instancia
# with open("Escenarios/Escenario_3.pkl", "wb") as f:
#     pickle.dump(E1, f)
#--------------------------------------------  
# ====== Solución ======
res = E3.solve(MaxRuntime(300))
print(res.best)
solution = res.best
with open("Escenarios/Res3.pkl", "wb") as f:
    pickle.dump(res, f)
#--------------------------------------------  
# ====== Mapa Rutas ======
m = E3
