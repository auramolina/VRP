...
for frm_node in locations.keys():
    for to_node in locations.keys():
        ...
        dist, dur = matrices(frm_id, to_id)
...
print([e.distance for e in m._edges])
#--------------------------------------------  
# Guardar instancia
# Exportar en .pkl
# problem = m.data()
# with open("edges/problem-g.pkl", "wb") as f:
#     pickle.dump(problem, f)
# with open("edges/modelo-g.pkl", "wb") as f:
#     pickle.dump(m, f)
#--------------------------------------------
from pyvrp import solve
from pyvrp.stop import MaxRuntime
# res = m.solve(MaxRuntime(20))
# print(res.best)

# edges_geojson = sorted([(e.frm, e.to, e.distance, e.duration) for e in m._edges])
# print(edges_geojson)